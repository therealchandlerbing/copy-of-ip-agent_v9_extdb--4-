...
INSTRUCTIONS:
1. OVERVIEW (MINIMUM 600 WORDS): Deconstruct the system architecture in high detail.
   - Explain the "Why" and "How" of the physics/logic involved.
   - Analyze the "Energy Budget", "Compute Budget", or "Mass Budget".
   - Provide at least 5-7 core features, each with detailed descriptions (100+ words each).
2. CORE TECHNOLOGY (MINIMUM 400 WORDS): Explain the "Magic" mechanism in deep technical terms.
   - SPECIFICATIONS: Provide 7-10 specific parameter estimates based on physics/industry standards.
   - Each specification must include detailed notes (50+ words) explaining its significance.
   - BENCHMARK: You MUST compare against the absolute best competitor for each parameter.
3. CLAIMS MATRIX: Extract 6-8 inventor promises.
   - INTERROGATE each claim with detailed evidence analysis (100+ words per claim).
   - Is there peer-reviewed evidence? Cite specific sources or explain gaps.
4. TECHNICAL RISKS (Physics of Failure):
   - Identify 6-8 failure modes spanning all risk levels.
   - Each description must be 80+ words explaining the specific failure mechanism.
   - Each mitigation must be 80+ words detailing the ENGINEERING solution.
5. TRL ASSESSMENT (MINIMUM 300 WORDS for overallAssessment): Be conservative.
   - Provide detailed justification based on NASA/DoD definitions.
   - Assess 5-8 subsystems with comprehensive analysis for each.
   - Each subsystem must have detailed status and evidence (80+ words).
6. VALIDATION GAPS (MINIMUM 200 WORDS for intro): What test is missing?
   - Identify 5-8 critical testing gaps with specific test standards.
   - Each gap description must be comprehensive (60+ words).

OUTPUT JSON FORMAT:
{
  "overview": { "paragraph": "string (Deep technical breakdown with **bolding**, 600+ words)", "coreFeatures": [{ "name": "string", "description": "string (100+ words each, provide 5-7 features)" }] },
  "coreTechnology": {
    "explanation": "string (Physics-based explanation, 400+ words, use **bolding**)",
    "specifications": [{ "parameter": "string", "value": "string (Estimated)", "benchmark": "string (SOTA Comparison)", "notes": "string (50+ words explaining significance, provide 7-10 specs)" }]
  },
  "claimsMatrix": [{
    "claimName": "string",
    "inventorAssertion": "string (60+ words)",
    "evidence": "string (100+ words with detailed analysis)",
    "validationSource": "string (e.g., 'Physics Calculation', 'Literature Review')",
    "evidenceTier": number,
    "confidence": "High|Medium|Low|Unvalidated"
  }],
  "technicalRisks": [{ "component": "string", "riskLevel": "high|medium|low", "description": "string (80+ words explaining failure mechanism, provide 6-8 risks)", "mitigation": "string (80+ words detailing engineering solution)" }],
  "trlAssessment": {
    "overallTrl": number,
    "overallAssessment": "string (300+ words with detailed justification based on NASA/DoD definitions)",
    "subsystems": [{ "name": "string", "trl": number, "status": "string (80+ words)", "evidence": "string (80+ words)", "gapToTrl6": "string (60+ words, provide 5-8 subsystems)" }]
  },
  "validationGaps": {
    "intro": "string (200+ words explaining testing philosophy and gaps overview)",
    "gaps": [{ "name": "string", "requiredTesting": "string (Specific Test Standard e.g., ASTM/ISO with 60+ words detail)", "estimatedCost": "string", "timeline": "string", "priority": "high|medium|low" }]
  }
}
`;
...
INSTRUCTIONS:
1. SEARCH STRATEGY (MINIMUM 300 WORDS for intro): Define at least 5-6 specific CPC/IPC subclasses.
   - For each component, provide comprehensive search terms and detailed results analysis (80+ words each).
   - Gaps analysis must be thorough (150+ words).
2. CLASSIFICATION ANALYSIS (MINIMUM 400 WORDS): Deep analysis of patent landscape density.
   - Analyze 5-7 classification codes with detailed strategic implications (80+ words each).
3. BLOCKING PATENTS:
   - Identify 6-8 REAL, high-threat assignees with ACTUAL patent numbers.
   ...
   - For every patent, provide detailed relevance analysis (100+ words).
   - Claims coverage must be comprehensive (100+ words per patent).
   - Differentiation opportunities must be detailed (100+ words per patent).
4. WHITESPACE (MINIMUM 500 WORDS for description): Detailed technical description of the open lane.
   - Intro must be 150+ words.
   - Evidence section must be 200+ words with specific data.
   - Each strategic partnership element must be 100+ words.
5. FTO ASSESSMENT (MINIMUM 400 WORDS for overallAssessment): Legal opinion summary.
   - Analyze 6-8 components with detailed risk analysis and mitigation (100+ words each).
6. FILING STRATEGY (MINIMUM 250 WORDS for priorityClaims): Propose a comprehensive "Picket Fence" strategy.
   - Define 4-6 filing phases with extensive details (100+ words each).
   - List 8-12 specific items to patent protect.
   - List 5-8 specific trade secrets.

OUTPUT JSON FORMAT:
{
  "searchMethodology": {
    "intro": "string (300+ words explaining search philosophy and approach)",
    "components": [{ "component": "string", "searchTerms": "string (comprehensive list)", "databases": "string", "dateRange": "string", "resultsCount": "string (with 80+ words analysis, provide 5-6 components)" }],
    "gaps": "string (150+ words on search limitations and areas of uncertainty)"
  },
  "classificationAnalysis": "string (400+ words deep analysis of patent landscape density, trends, and implications)",
  "classificationCodes": [{ "code": "string (CPC/IPC)", "description": "string (60+ words)", "strategicImplication": "string (80+ words, provide 5-7 codes)" }],
  "whitespace": {
    "intro": "string (150+ words on whitespace opportunity)",
    "description": "string (500+ words detailed technical description with **bolding**)",
    "evidence": "string (200+ words with specific search data and analysis)",
    "strategicPartnerships": {
        "licensingTargets": "string (100+ words on specific companies and why)",
        "partnershipModel": "string (100+ words on commercial structure)",
        "rationale": "string (100+ words on strategic value)"
    }
  },
  "blockingPatents": [{
    "patentNumber": "string",
    "holder": "string",
    "expiration": "string",
    "relevance": "string (100+ words detailed analysis)",
    "ftoRisk": "high|medium|low",
    "claimsCoverage": "string (100+ words on specific claim elements that overlap)",
    "differentiationOpportunity": "string (100+ words on how to engineer around it, provide 6-8 patents)"
  }],
  "ftoAssessment": {
    "components": [{ "component": "string", "riskLevel": "high|medium|low|zero", "blockingPatents": "string (80+ words)", "mitigation": "string (100+ words with specific strategies, provide 6-8 components)" }],
    "overallAssessment": "string (400+ words legal opinion summary with specific recommendations)"
  },
  "filingStrategy": {
    "phases": [{ "name": "string", "timeline": "string", "focus": "string (60+ words)", "cost": "string", "details": "string (100+ words on specific claims and approach, provide 4-6 phases)" }],
    "priorityClaims": "string (250+ words on priority claim strategy and rationale)",
    "patentProtect": ["string (8-12 specific items with technical detail)"],
    "tradeSecrets": ["string (5-8 specific items with rationale)"]
  }
}
`;
...
INSTRUCTIONS:
1. MARKET SIZING:
   - Provide specific dollar amounts ($B) with detailed methodology (200+ words).
   - List 5-6 specific "Key Drivers" (each 50+ words explaining impact).
   - Identify 5-6 "Market Trends" (each with 80+ words impact analysis).
2. THE GRAVEYARD (MINIMUM 250 WORDS for intro): Identify *specific* failed startups. Why did they die?
   - Provide 4-6 detailed examples.
   - Each failure mode must be comprehensive (100+ words).
   - Each lesson must be actionable (80+ words).
3. ZOMBIE COMPETITORS (MINIMUM 200 WORDS for intro): Companies showing decline signs.
   - Identify 3-5 zombie companies.
   - Each warning sign analysis must be detailed (80+ words).
4. COMPETITORS:
   - Identify 8-12 Incumbents and Challengers across all statuses.
   - Each competitor must have detailed value proposition (100+ words).
   - Each vulnerability must be comprehensive (100+ words).
5. FEATURE WAR ROOM:
   - List 8-12 Critical Features with detailed advantage analysis (60+ words each).
6. GAP ANALYSIS (MINIMUM 300 WORDS for intro, 250+ WORDS for unmetDemandEvidence):
   - Identify 6-8 specific market gaps with detailed explanations (each 80+ words).
7. BEACHHEAD MARKET (200+ WORDS for each field):
   - Define the *exact* first customer profile with demographics and psychographics.
   - Explain specific "Pain Point" with quantified impact.
   - Detail tolerance reasoning with competitive analysis.
   - Provide expansion path with specific milestones.
8. CUSTOMER ACQUISITION: Define 5-7 acquisition milestones.
   - Each strategy must be detailed (100+ words).
9. MONETIZATION: Analyze 4-6 pricing tiers/models.
   - Each with comprehensive value proposition (100+ words).

OUTPUT JSON FORMAT:
{
  "marketSizeAnalysis": {
    "totalAddressableMarket": "string (with 200+ words methodology and data sources)",
    ...
    "keyDrivers": ["string (5-6 drivers, each 50+ words explaining impact)"],
    "marketTrends": [{ "trend": "string", "impact": "string (80+ words detailed impact analysis, provide 5-6 trends)" }]
  },
  "graveyard": {
    "intro": "string (250+ words on failure patterns and lessons)",
    "failedProducts": [{ "name": "string", "company": "string", "timeline": "string", "failureMode": "string (100+ words on why they failed)", "lesson": "string (80+ words actionable lesson, provide 4-6 examples)" }]
  },
  "zombieCompetitors": {
    "intro": "string (200+ words on decline indicators in the sector)",
    "companies": [{ "name": "string", "lastUpdate": "string", "warningSigns": "string (80+ words detailed analysis)", "implication": "string (80+ words, provide 3-5 companies)" }]
  },
  "competitiveLandscape": [{
    "name": "string", "segment": "string", "geography": "string", "valueProposition": "string (100+ words)", "vulnerability": "string (100+ words detailed weakness analysis)", "status": "Active|Zombie|Exited (provide 8-12 competitors)"
  }],
  "featureComparison": [{
     "feature": "string",
     "us": boolean,
     "competitor": boolean,
     "advantage": "string (60+ words explaining competitive advantage, provide 8-12 features)"
  }],
  "gapAnalysis": { "intro": "string (300+ words on market analysis approach)", "gaps": ["string (6-8 gaps, each 80+ words with specific unmet needs)"], "unmetDemandEvidence": "string (250+ words with specific data, surveys, quotes)" },
  "beachheadMarket": { "profile": "string (200+ words with specific demographics)", "painPoint": "string (200+ words with quantified impact)", "toleranceReason": "string (200+ words on why they'll pay premium)", "marketSize": "string (with detailed calculation)", "expansionPath": "string (200+ words with specific milestones and timeline)" },
  "customerAcquisition": [{ "milestone": "string", "strategy": "string (100+ words detailed strategy)", "channels": "string (specific channels with rationale)", "timeline": "string (provide 5-7 milestones)" }],
  "monetization": { "pricingAnalysis": [{ "category": "string", "priceRange": "string", "valueProposition": "string (100+ words justifying price point, provide 4-6 pricing tiers)" }] }
}
`;
...
INSTRUCTIONS:
1. CLASSIFICATION (MINIMUM 300 WORDS for intro): Be precise (FDA Codes, ISO Standards).
   - Provide detailed regulatory classification with rationale (150+ words).
   - Explain pathway selection with precedents (150+ words).
   - List 6-10 applicable standards with detailed explanations of each.
2. PREDICATES: Identify 5-8 existing certified products.
   - Each must have comprehensive relevance analysis (100+ words).
   - Include detailed review timeline breakdown.
3. TIMELINE & COST: Be realistic and comprehensive.
   - Define 6-10 regulatory phases from concept to approval.
   - Each phase must have detailed activities breakdown (100+ words).
   - Include cost ranges with justification.
4. RECENT DEVELOPMENTS (MINIMUM 200 WORDS for intro): Regulatory landscape analysis.
   - Identify 5-8 recent regulatory changes or guidances.
   - Each impact analysis must be detailed (80+ words).
5. RISKS: What tests are most likely to fail?
   - Identify 6-8 regulatory risks across all phases.
   - Each description must be comprehensive (100+ words).
   - Each mitigation must be actionable and detailed (100+ words).

OUTPUT JSON FORMAT:
{
  "classification": {
    "intro": "string (300+ words explaining regulatory landscape for this technology)",
    "regulatoryClassification": "string (150+ words with specific codes and rationale)",
    "pathway": "string (150+ words explaining pathway selection with precedents)",
    "timelineEstimate": "string (with detailed breakdown and assumptions)",
    "standards": ["string (6-10 standards with detailed explanation of each requirement)"]
  },
  "comparableSystems": [{
    "productName": "string",
    "referenceNumber": "string",
    "approvalDate": "string",
    "relevance": "string (100+ words detailed comparison to our technology)",
    "reviewTime": "string (with phase breakdown, provide 5-8 comparable systems)"
  }],
  "timelineCost": [{
    "phase": "string",
    "activities": "string (100+ words detailed breakdown of activities and deliverables)",
    "duration": "string (with range and assumptions)",
    "cost": "string (with itemized estimate, provide 6-10 phases)"
  }],
  "recentDevelopments": {
    "intro": "string (200+ words on evolving regulatory landscape)",
    "developments": [{
      "title": "string",
      "date": "string",
      "impact": "string (80+ words on how this affects approval pathway, provide 5-8 developments)"
    }]
  },
  "risks": [{
    "title": "string",
    "description": "string (100+ words on specific regulatory failure mode)",
    "mitigation": "string (100+ words with actionable strategies, provide 6-8 risks)"
  }]
}
`;
...
1. ACTION PLAN: Create comprehensive development roadmap.
   - Define 8-12 time-based phases from concept to commercialization.
   - Each phase must have detailed activities (100+ words).
   - Each milestone must have clear success criteria (80+ words).
2. BUDGET FRAMEWORK: Break down all cost categories.
   - Provide 8-12 budget categories with detailed allocation rationale.
   - Each category must explain activities in detail (100+ words).
3. UNIT ECONOMICS (The BOM):
   - Break down the COGS into 10-15 detailed line items.
   - Each component must have comprehensive notes (80+ words) on sourcing, risks, and scaling.
   - Include detailed breakeven analysis (200+ words) with sensitivity analysis.
   - Provide volume-based COGS projections at 3-5 different production scales.
4. FUNDING (COMPREHENSIVE ANALYSIS):
   - Seed Round: Detailed requirements (200+ words on what we need to prove).
   - List 8-12 specific use of funds items with detailed justification.
   - Series A: Detailed milestone analysis (200+ words).
   - List 8-12 specific use of funds items with ROI projections.
   - Include sources with detailed investor targeting strategy.
5. TEAM GAPS: Comprehensive hiring roadmap.
   - Identify 8-12 critical roles with detailed job descriptions.
   - Each must have comprehensive budget impact analysis (80+ words).
...
OUTPUT JSON FORMAT:
{
  "actionPlan": [{
    "months": "string",
    "phase": "string",
    "activities": "string (100+ words with specific deliverables and tasks)",
    "budget": "string (with itemized breakdown)",
    "milestone": "string (80+ words with clear success criteria, provide 8-12 phases)"
  }],
  "budgetFramework": [{
    "category": "string",
    "allocation": "string (with justification)",
    "activities": "string (100+ words on what this funds and why, provide 8-12 categories)"
  }],
  "unitEconomics": {
    "bom": [{
      "component": "string",
      ...
      "notes": "string (80+ words on sourcing strategy, risks, and scaling considerations, provide 10-15 components)"
    }],
    "totalCogs": "string (with detailed calculation breakdown)",
    "cogsVolume": "string (provide 3-5 volume tiers with corresponding COGS)",
    "targetAsp": "string (with pricing strategy rationale, 150+ words)",
    "grossMargin": "string (with sensitivity analysis and assumptions)",
    "breakevenUnits": "string (200+ words with detailed breakeven analysis, time to breakeven, and sensitivity to key assumptions)"
  },
  "fundingRequirements": {
    "seed": {
      "amount": "string (with range and justification)",
      "useOfFunds": ["string (8-12 detailed line items with specific amounts and ROI projections)"],
      "sources": "string (200+ words on investor targeting strategy, types of investors, and approach)"
    },
    "seriesA": {
      "amount": "string (with range and justification)",
      "useOfFunds": ["string (8-12 detailed line items with specific amounts and growth projections)"],
      "trigger": "string (200+ words on specific milestones that unlock Series A, with metrics and timing)"
    }
  },
  "teamGaps": [{
    ...
    "budgetImpact": "string (80+ words on fully-loaded cost, ROI, and alternatives, provide 8-12 roles)"
  }]
}
`;
...
INSTRUCTIONS:
1. RISK PROFILE (MINIMUM 800 WORDS): Synthesize all constraints comprehensively. Professional tone.
...
2. CRITICAL CONCERNS: Identify 8-12 major hurdles.
   - Each "what" field must be detailed (100+ words).
   - Each "whyItMatters" must quantify impact (100+ words).
   - Each "resolution" must be actionable and comprehensive (150+ words).
3. KEY STRENGTHS: Identify 6-10 competitive advantages.
   - Each description must be detailed (100+ words).
   - Each evidence section must cite specific data (100+ words).
4. COMMERCIALIZATION PATH (MINIMUM 400 WORDS for narrative):
   - Provide comprehensive go-to-market strategy.
   - Detail specific milestones with timeline and success criteria.
5. DATA CONFIDENCE: Assess 6-8 key areas.
   - Each known gaps analysis must be detailed (100+ words).
6. PRIORITY ACTIONS: Define 10-15 critical next steps.
   - Each action must be detailed with specific deliverables (100+ words).
7. PARTNERSHIPS: Identify 5-8 strategic partnership opportunities.
   - Each targets section must name specific companies (100+ words).
   - Each approach must be tactical and detailed (100+ words).
8. ALTERNATIVE PATHWAYS (MINIMUM 300 WORDS for intro): Explore strategic options.
   - Provide 4-6 alternative paths with comprehensive analysis.
   - Each description must be detailed (150+ words).
   - Each tradeoffs analysis must be thorough (150+ words).
9. GO/NO-GO FRAMEWORK: Create comprehensive decision framework.
   - Provide 8-12 go criteria with specific metrics.
   - Provide 6-10 no-go criteria with thresholds.
10. MONITORING METRICS: Define 10-15 key performance indicators.
    - Each significance analysis must be detailed (80+ words).
11. RISK INDICATORS: List 12-20 specific red flags to watch.
12. TTO DIRECTOR'S INSIGHTS (The Memo):
    - Standalone Memorandum (MINIMUM 800 WORDS).
    - Focus: Asset transferability, licensing viability, strategic positioning.
    - Provide 6-10 "Strategic Mandates" with detailed justification.
...
OUTPUT JSON FORMAT:
{
  "executiveSummary": {
    ...
    "riskProfile": {
      ...
      "tier2Count": number,
      "tier3Count": number,
      "summaryParagraph": "string (MINIMUM 800 words. Use **bolding**. Comprehensive synthesis integrating all dimensions with specific quantified impacts and cross-functional risk analysis.)",
      "recommendation": "string (Short, punchy verdict e.g. 'CONDITIONAL PROCEED')",
      "keyCondition": "string (Detailed description of the single most important condition, 100+ words)",
      "investmentRequired": "string (Detailed breakdown of total capital required with phasing and use of funds)"
    },
    "criticalConcerns": [{
      "title": "string",
      "what": "string (100+ words detailed description)",
      "whyItMatters": "string (100+ words quantifying business impact)",
      "resolution": "string (150+ words with actionable steps and timeline)",
      "tier": 1|2
    }],
    "keyStrengths": [{
      "title": "string",
      "description": "string (100+ words explaining the competitive advantage)",
      "evidence": "string (100+ words citing specific data, comparisons, and validation, provide 6-10 strengths)"
    }],
    "commercializationPath": {
      ...
      "narrative": "string (MINIMUM 400 words comprehensive go-to-market strategy with specific tactical steps)",
    ...
    "dataConfidence": [{
      ...
      "knownGaps": "string (100+ words on what data is missing and how to obtain it, provide 6-8 areas)"
    }]
  },
  "strategicRecommendations": {
    "priorityActions": [{
      "action": "string (100+ words with specific deliverables and success criteria)",
      "owner": "string (role description)",
      "timeline": "string (detailed with milestones)",
      "budget": "string (itemized estimate, provide 10-15 actions)"
    }],
    "partnerships": [{
      "type": "string (partnership model)",
      "targets": "string (100+ words naming 3-5 specific companies with rationale)",
      "valueExchange": "string (80+ words on what each party contributes)",
      "approach": "string (100+ words tactical outreach strategy, provide 5-8 partnership types)"
    }],
    "alternativePathways": {
      "intro": "string (300+ words on strategic options philosophy)",
      "alternatives": [{
        "name": "string",
        "description": "string (150+ words comprehensive description with specific steps)",
        "tradeoffs": "string (150+ words detailed pros/cons analysis, provide 4-6 alternatives)"
      }]
    },
    "goNoGoFramework": {
      "checkpointTimeline": "string (detailed timeline with specific decision points)",
      "goCriteria": ["string (8-12 specific criteria with metrics and thresholds)"],
      "noGoCriteria": ["string (6-10 specific red lines with clear thresholds)"]
    },
    "monitoringMetrics": [{
      "category": "string",
      "metric": "string (specific KPI with measurement method)",
      "significance": "string (80+ words on why this matters and how to interpret, provide 10-15 metrics)"
    }],
    "riskIndicators": ["string (12-20 specific early warning signals to monitor continuously)"],
    "ttoSynthesis": {
       "insightNarrative": "string (The 'Director's Memo' - blunt, strategic advice. MINIMUM 800 WORDS. Use highly professional, authoritative tone. Cover asset transferability, licensing strategy, strategic positioning, market timing, and partnership approach with specific recommendations.)",
       "keyRecommendations": [{
         "title": "string",
         "description": "string (150+ words with specific action steps and rationale)",
         "priority": "Critical|High|Medium (provide 6-10 strategic mandates)"
       }]
    }
  }
}
...
export class GeminiService {
  ...
  private async runAssessmentPipeline(responses: any, inputType: InputType): Promise<AssessmentReport> {
    ...
    // Increased Thinking Budgets for Deep Analysis - Higher budgets for 40-50 page reports
    const [ipDeepDive, marketDynamicsResponse, visualConcept] = await Promise.all([
      this.executeDirective("IP Analysis", PATENT_ATTORNEY_PROMPT, sectorContext, DEFAULT_IP_DEEP_DIVE, true, 8192),
      this.executeDirective("Market Dynamics", MARKET_STRATEGIST_PROMPT, sectorContext, DEFAULT_MARKET_DYNAMICS, true, 8192),
    ...
    const technologyForensics = await this.executeDirective("Technical Validation", TECHNOLOGIST_PROMPT, { ...sectorContext, subsystems: ipDeepDive.searchMethodology?.components }, DEFAULT_TECH_FORENSICS, true, 8192);
    ...
    const financialRoadmap = await this.executeDirective("Financials", COMMERCIAL_LEAD_PROMPT, { ...sectorContext, market: marketDynamics, reg: regulatoryPathway }, DEFAULT_FINANCIAL, false, 6144);
    ...
    const synthesis = await this.executeDirective("Synthesis", SYNTHESIS_PROMPT, synthesisContext, DEFAULT_SYNTHESIS, false, 12288);
